...
class BookingManager:
    def __init__(self):
        ...
        self.routing_accuracy = {
            ...
            "urgent_requests_handled": 0  # Added for tracking urgent requests
        }
        # Customer profiles
    ...
    def add_booking(self, service_type, customer_name, expected_service=None, is_urgent=False):
        ...
        urgent_handling = ""
        if is_urgent:
            urgent_handling = " as URGENT PRIORITY (紧急优先处理)"
            self.routing_accuracy["urgent_requests_handled"] += 1
        
        # Random branch assignment
        branch = random.choice(self.locations)
        
        # Return bilingual response with urgency status if applicable
        return f"{customer_name}'s {service_type} service booking is confirmed at {branch}{special_handling}{urgent_handling}. ({customer_name}的{service_type}服务预约已确认，地点在{branch}{special_handling}{urgent_handling}。)"
...
@tool
def handle_deposit(customer_name: str, is_urgent: bool = False) -> str:
    ...
    """
    return booking_manager.add_booking("deposit", customer_name, "deposit", is_urgent)

@tool
def handle_postal(customer_name: str, is_urgent: bool = False) -> str:
    ...
    """
    return booking_manager.add_booking("postal", customer_name, "postal", is_urgent)

@tool
def handle_loan(customer_name: str, is_urgent: bool = False) -> str:
    ...
    """
    return booking_manager.add_booking("loan", customer_name, "loan", is_urgent)

@tool
def handle_bill_payment(customer_name: str, is_urgent: bool = False) -> str:
    ...
    """
    return booking_manager.add_booking("bill_payment", customer_name, "bill_payment", is_urgent)

@tool
def handle_international_transfer(customer_name: str, is_urgent: bool = False) -> str:
    ...
    """
    return booking_manager.add_booking("international_transfer", customer_name, "international_transfer", is_urgent)

@tool
def handle_general_inquiry(customer_name: str, is_urgent: bool = False) -> str:
    ...
    # Track urgent handling if applicable
    if is_urgent:
        booking_manager.routing_accuracy["urgent_requests_handled"] += 1
        return f"{customer_name} has been redirected to the general information desk with URGENT priority. (已将{customer_name}紧急优先转接至一般咨询服务台。)"
...
class EnhancedChineseBankPostOfficeAgent(ToolCallingAgent):
    """Enhanced agent for the Chinese Bank Post Office that includes urgency detection."""

    def __init__(self, model: OpenAIServerModel):
        self.urgency_detector = UrgencyDetector(model)
...
def print_state():
    ...
    print(f"  - Urgent Requests Handled: {booking_manager.routing_accuracy['urgent_requests_handled']} times")
    
    # Overall success assessment
    if (accuracy >= 80 and 
        booking_manager.routing_accuracy["special_handling_applied"] >= 2 and
        booking_manager.routing_accuracy["urgent_requests_handled"] >= 1):
        print("\n✅ SUCCESS: The system demonstrated proper routing, special case handling, and urgency detection!")
    else:
        print("\n⚠️ The system needs improvement in request routing, special handling, or urgency detection.")
    print("=" * 80)

if __name__ == "__main__":
    # Use the enhanced agent that includes urgency detection
    bank_post_office_agent = EnhancedChineseBankPostOfficeAgent(model)
    
    print("🏦 Enhanced Chinese Postal Bank Service Demo (增强版中国邮政银行服务示例) 🏦\n")
    print("This demo demonstrates proper routing, data flow, and urgency detection between agents.\n")
    ...
    for case in test_cases:
        ...
        # Run the request with enhanced agent
